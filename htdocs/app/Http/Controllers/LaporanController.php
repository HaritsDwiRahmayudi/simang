...
class LaporanController extends Controller
{
    ...
    public function index()
    {
        ...
        // --- GEMBOK HAK AKSES ---
        // Kalau belum diapprove, tendang ke dashboard
        if ($magang->status != 'approved') {
            return redirect()->route('dashboard')->with('error', 'Akun magang belum disetujui! Anda belum bisa mengakses Laporan/Logbook.');
        }
        // ------------------------
   ...
   public function upload(Request $request)
    {
        // --- GEMBOK LAPIS DUA (Anti By-pass API) ---
        $userId = Auth::id();
        $magang = Magang::where('user_id', $userId)->first();
        if (!$magang || $magang->status != 'approved') {
            return redirect()->route('dashboard')->with('error', 'Akses ditolak! Akun magang belum disetujui.');
        }
        // -------------------------------------------
    ...
    public function downloadPdf(Request $request)
    {
        ...
        // --- GEMBOK LAPIS TIGA (Proteksi Cetak) ---
        if (!$magang || $magang->status != 'approved') {
            return redirect()->route('dashboard')->with('error', 'Akun magang belum disetujui! Anda belum bisa mencetak jurnal.');
        }
        // ------------------------------------------
