                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                <tr class="hover:bg-blue-50 transition duration-200">
                                    ...
                                    <td class="px-6 py-4 align-top text-center">
                                        @if($log->foto_kegiatan)
                                            <a href="{{ asset('arsip/' . $log->foto_kegiatan) }}" target="_blank" class="inline-block relative group">
                                                <img src="{{ asset('arsip/' . $log->foto_kegiatan) }}" class="h-12 w-12 rounded-lg object-cover border border-gray-300 shadow-sm transition transform group-hover:scale-150 group-hover:z-50 group-hover:border-white group-hover:shadow-2xl">
                                            </a>
                                        @else
                                    ...
                                    <td class="px-6 py-4 align-top text-center">
                                        @if($log->ttd_koordinator)
                                            <img src="{{ asset('arsip/' . $log->ttd_koordinator) }}" class="h-12 w-auto mx-auto border border-gray-200 rounded p-1 bg-white">
                                        @else
                                    ...
                                    <td class="px-6 py-4 align-top text-center">
                                        <div class="flex flex-col items-center space-y-2">
                                            <a href="{{ route('logbook.edit', $log->id) }}" class="w-full text-center bg-yellow-500 hover:bg-yellow-600 text-white text-xs font-bold py-1.5 px-3 rounded shadow-sm transition">
                                                Edit
                                            </a>
                                            <form action="{{ route('logbook.destroy', $log->id) }}" method="POST" class="w-full" onsubmit="return confirm('Yakin mau hapus data logbook ini? Foto dan Paraf juga akan terhapus lho!')">
                                                @csrf
                                                @method('DELETE')
                            ...
                            @empty
                                <tr>
                                    {{-- Colspan diubah dari 5 jadi 6 karena nambah kolom Aksi --}}
